...
声明：  
&emsp;&emsp;本篇博客用来解决日常生产开发中的一些常见需求，重点在思路讲解而非执行计划调优，所以性能考虑不在第一优先级。  
...
举个例子，假设有两张表，一张是安装表，一张是注册表，两张表用设备id关联。假设不考虑一个设备的多次安装或多次注册，即设备只有一个设备id，一个用户只有一个设备。  
...
### 如何用一条 SQL 算出多天的快照？ {id="sql_5_4"}

上面的 SQL 只能查询 `2026-01-10` 这天的快照，如果要查 `2026-01-08` 的快照，就只能手改 SQL 再跑一次。

那么能不能用一条 SQL 就能查出 `2026-01-01` 到 `2026-01-10` 的快照呢？可以的
...
即 <math>S(t-1)</math> 直接直接读取昨日保存的快照结果，并累加今日增量事件 <math>\Delta(t)</math>，最终得到今天的快照 <math>S(t) = S(t-1) + \Delta(t)</math>。

最后让我们用 CTE 将上面的 SQL 改成更为清晰一点的写法
